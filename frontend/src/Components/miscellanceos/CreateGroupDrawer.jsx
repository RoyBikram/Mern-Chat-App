...
const CreateGroupDrawer = ({ DisplayDrawer, ToggleDrawer }) => {
  ...
  const handleChipDelete = (user) => {
    setSelectedUsers((SelectedUsers) =>
      SelectedUsers.filter((Each) => Each._id !== user._id)
    );
  };
  const handleCreateGroupChat = async (event) => {
    // To Work The Required Field
    formRef.current.reportValidity();
    event.preventDefault();
    try {
  ...
  const handleNextButtonClick = () => {
    if (SelectedUsers.length < 2) {
      ...
      console.log("At-least need Two user");
      return;
    }
    setSearchResult([]);
    setTabState(2);
  ...
  const addUserToGroup = (AddUser) => {
    if (SelectedUsers.find((each) => each._id === AddUser._id)) {
